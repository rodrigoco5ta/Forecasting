...
for (i in 1:m){
  Sline[i] = mean(as.numeric(get_elements(Dif, i, m)))
  SDif[i] = sd(as.numeric(get_elements(Dif, i, m)))
}
...
LowBound = Sline - 2 * SDif
UpBound = Sline + 2 * SDif


#Check if value is between range
for (i in (m/2+1):(n-m/2)){
  if (Dif[i] >= LowBound[i] & Dif[i] <= UpBound[i]){
    OK[i] = "OK"
  } else {
    OK[i] = "NOT OK"
  }
...
Seasonality = Sline - sum_Sline/m


#### CALCULATE THE ERROR - INHERENT VARIABILITY ####
Errort = rep(NA, n)
for (i in (m/2+1):(n-m/2)){
  Errort[i] = Dif[i]-Seasonality[i]
}
...
stat_test = (average_errors - 0) / (sdev_errors / sqrt(n-m))
...
#### CONVERT INTO EXCEL ####
write_xlsx(df, "AddModel_R.xlsx")
write_xlsx(stats, "Stats_Add_R.xlsx")
